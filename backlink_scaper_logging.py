...
# Logging einrichten
logging.basicConfig(
    filename='scraper_log.txt',  # Name der Logdatei
...
def scrape_backlinks(source_url):
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            # Filterung von Navigationslinks
            if anchor_text in navigation_keywords or any(nav_path in target_url for nav_path in navigation_paths):
                continue  # Navigationslinks ignorieren

            # Nur valide Links weiterverarbeiten
            if not (target_url.startswith('/') or target_url.startswith('.')):
                logging.info(f"Valid URL gefunden: {target_url}, Anchor: {anchor_text}")
