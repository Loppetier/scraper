...
# Fortschrittsanzeige mit Lock
progress_lock = Lock()
processed_urls = set()
total_urls = 0

def show_progress(total, current):
    """Thread-sicherer Fortschrittsanzeiger."""
    with progress_lock:
        ...
        print(f"Verbleibende Zeit: {datetime.timedelta(seconds=int(remaining_time))}")
...
# URLs validieren (nur V3-Onion-Adressen und keine internen Pfade)
...
def categorize_url_from_html(soup, default_category="Unknown"):
    ...
    if any(keyword in title or keyword in text_content for keyword in ["market", "shop", "buy"]):
        return "Market"
    elif any(keyword in title or keyword in text_content for keyword in ["forum", "discussion"]):
        return "Forum"
    elif any(keyword in title or keyword in text_content for keyword in ["mail", "email"]):
        return "Email Service"
    elif any(keyword in title or keyword in text_content for keyword in ["news", "article"]):
        return "News"
    elif any(keyword in title or keyword in text_content for keyword in ["search", "find"]):
...
def start_scraping(urls):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        executor.map(scrape_backlinks, urls)
...
start_scraping(urls_to_process)
