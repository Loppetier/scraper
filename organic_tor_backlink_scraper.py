...
def fetch_html(url):
    """Ruft den HTML-Inhalt einer Seite ab."""
    try:
...
def extract_onion_links(html, base_url):
    """Extrahiert .onion-Links und Anchor-Texte aus dem HTML."""
...
def save_to_database(source_url, backlinks):
    """Speichert die Backlinks in der SQLite-Datenbank."""
    try:
        # Quelle speichern
        ...
        for target_url, anchor_text in backlinks:
            # Zielseite speichern
            ...
            # Backlink speichern
            cursor.execute('''
                INSERT INTO backlinks (source_id, target_id, anchor_text)
                VALUES (?, ?, ?)
